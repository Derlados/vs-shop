...
const SliderProducts: FC<SliderProductsProps> = observer(({ products, title, slidesPerView = 6 }) => {
  ...
  useEffect(() => {
    window.addEventListener("resize", () => {
      ...
      if (window.innerWidth <= Resolutions.MOBILE_VERTICAL_LARGE && slidesPerView > MAX_PER_MOBILE_VERTICAL) {
  ...
  return (
    <div className='slider-products clc'>
      {localStore.selectedProduct &&
        <ProductQuickModal
          isOpen={localStore.isOpenQuick}
          product={localStore.selectedProduct}
